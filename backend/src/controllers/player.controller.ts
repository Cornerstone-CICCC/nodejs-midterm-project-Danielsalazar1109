...
export const updatePlayerById = (req: Request, res: Response): void => {
  const { id } = req.params;
  ...
  if (playerIndex === -1) {
    ...
    players[playerIndex] = { ...players[playerIndex], name, position, number, captain, teamId };
    res.json(players[playerIndex]);
  }
};
export const deletePlayerById = (req: Request, res: Response): void => {
  const { id } = req.params; 
  const playerIndex = players.findIndex(player => player.id === id);
